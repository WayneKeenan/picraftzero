...
class RemoteCommands:
    ...
    def run_remote_command(self, host, command, result):
        ...
        logging.debug("run_remote_command: {}".format([self.user, host, command]))
        ...
        #logging.error("stderr; {}".format(errdata))
    ...
    def run_remote_commands(self, hosts, command):
        ...
        for host in hosts:
            ...
            logging.debug("Host: {} Cmd: {}".format(host, command))
            t = threading.Thread(target=self.run_remote_command, args=(host, command, results), name=host)
            threads.append(t)
            t.start()

        for t in threads:
            logging.info('Waiting for %s', t.getName())
            t.join(self.timeout)
            logging.info('Final status for {} = {}'.format(t.getName(), t.isAlive()))

        logging.info("Results: {}".format(pformat(results)))
        for host in hosts:
            if host in results:
                ...
                logging.error("No results found for {}".format(host))
...
class PiCraftServices:
    ...
    def hosts_for_service(self, service):
        ...
        for host, services in self.topology.items():
            if service in services:
                hosts.append(host)
        logging.info("Service {} is installed on : {}".format(service, hosts))
        return hosts
    ...
    def service_status(self):
        for service in self.all_services():
            ...
            logging.info("Getting status for service {} on hosts: {}".format(service, hosts))
...
if __name__ == "__main__":
    ...
    logging.info("All hosts                 : {}".format(pcs.all_hosts()))
    logging.info("All services              : {}".format(pcs.all_services()))
    logging.info("Hosts with camera services: {}".format(pcs.camera_hosts()))
    logging.info("Hosts with www services   : {}".format(pcs.www_hosts()))
